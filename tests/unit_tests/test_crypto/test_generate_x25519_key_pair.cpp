...
TEST( UnitTest, GenerateX25519KeyPair ) {
    auto first_key_pair_result = ntk::generate_x25519_key_pair();
    ...
    auto second_key_pair_result = ntk::generate_x25519_key_pair();
    ...
    auto client_secret_result = ntk::derive_x25519_shared_secret( client_private_key, server_public_key );
    ASSERT_TRUE( client_secret_result ) << client_secret_result.error() << std::endl;
    auto& client_secret = client_secret_result.value();
    auto server_secret_result = ntk::derive_x25519_shared_secret( server_private_key, client_private_key );
