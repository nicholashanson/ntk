...
TEST( UnitTest, TlsTransfer_Load ) {
    auto packet_data = ntk::read_packets_from_file( test::packet_data_files[ "tls_handshake" ] );
    auto four_tuples = ntk::get_four_tuples( packet_data );
    auto four_tuple = *four_tuples.begin();
    std::size_t data_packet_count = std::count_if( packet_data.begin(), packet_data.end(), [] ( const auto& packet ) {
                                                                                               auto result = ntk::is_data_packet( packet );
