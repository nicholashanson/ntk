...
TEST( UnitTest, GetStartOfTermination_TLSHandshake ) {
    ...
    auto termination_header_result = ntk::get_parsed_tcp_header_from_ethernet( std::get<ntk::rst>( termination_result.value().closing_sequence ) );
    ASSERT_TRUE( termination_header_result ) << termination_header_result.error() << std::endl;
    ...
    ASSERT_EQ( static_cast<int>( termination_header_result.value().seq_number ), 1441872756 );
}